...
async def test_add_message(db_session):
    ...
    msg = await repo.add_message(conversation_id=conv.id, role="user", content="Hello Friday!")
...
async def test_memory_repository(db_session):
    ...
    mem = await repo.store(user_id=user.id, category="preference", key="theme", value="dark")
